...
my $PMSAIKOTOBALIST_FILENAME = "pl_pwdlist.dat";
...
sub output_aikotoba_list {
	...
	#080625 tomoya すべての単語を重複文字のターゲットとする。
	#合言葉入力時に簡易会話入力画面では、重複文字の排除を行っていないため、
	#全単語の重複文字チェックを行います。
	#＊例として”きりふだ”が技名と重複しています。
...
#				print $datStrID[$i]." : ".$SrcString[$datStrID[$i]]."\n";
			...
			for ( $i=0; $i<@datStrID; $i++ ) {
				@aikotoba_cut_file = (@aikotoba_cut_file, $datStrID[$i] );
#				print $datStrID[$i]."cut : ".$SrcString[$datStrID[$i]]."\n";
			}
		}
	}

	@aikotoba_cut_file = sort {$a <=> $b} @aikotoba_cut_file;
...
#					print $SrcString[$strid_a]." eq ".$SrcString[$strid_b]."\n";
		...
		#その他の単語も含めた重複チェック	
		for( $j=0; $j<@aikotoba_cut_file; $j++ ){
	

			$strid_b = $aikotoba_cut_file[$j];
...
#			print $SrcString[$strid_a]." eq ".$SrcString[$strid_b]."\n";
	...
	print "->$PMSAIKOTOBA_FILENAME\n";
	close( FILEOUT );

	#boxpwd.pl用あいことば単語文字列リスト　書き出し
	open( FILEOUT, ">$PMSAIKOTOBALIST_FILENAME" );
	for( $i=0; $i<@aikotoba_file; $i++ ){
		print( FILEOUT $SrcString[$aikotoba_file[$i]]."\n" );
	}
	print "->$PMSAIKOTOBALIST_FILENAME\n";
	close( FILEOUT );
