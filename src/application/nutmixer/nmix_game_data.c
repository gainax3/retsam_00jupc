...
//----------------------------------------------------------------------------
/**
 *	@brief	元から入っているやつにMergeする
 *
 *	@param	p_data			データ
 *	@param	p_comm_pack		元からの通信データ
 *	@param	comm_count		通信人数
 */
//-----------------------------------------------------------------------------
void NMIX_DATA_OyaCommDataMerge( NMIX_DATA_OYA* p_data, NUTMIX_GAME_COMM_PACK* p_comm_pack, int comm_count )
{
	int i;
	NUTMIX_OYA* p_oya = &p_comm_pack->send_game_data.oya;
	s32	sp_rota;
	...
	// 焦げやこぼれのエフェクトデータは出来るだけ保持
	if( p_data->soop_speed_data.koge_eff != 0 ){
		p_oya->koge_req = p_data->soop_speed_data.koge_eff;
	}
	if( p_oya->kobore_req == FALSE ){
		p_oya->kobore_req = p_data->soop_speed_data.kobosi_eff;
	}

	// 回転リクエスト方向設定
	if( p_data->rota_waycont.rota_way_chg == TRUE ){
		p_oya->now_rota_req = p_data->rota_waycont.rota_way_req;
		p_oya->rota_req_eff = p_data->rota_waycont.rota_way_chg;
	}
