...
//#define DEBUG_PRINT_BATTLE	///<í“¬—pƒfƒoƒbƒOƒvƒŠƒ“ƒg‚ğ—LŒø‚É‚·‚éiƒƒ‚ƒŠƒI[ƒo[‚·‚é‚Ì‚ÅA—LŒø‚É‚Å‚«‚È‚¢j
#endif

#define BATTLE_SPEED_UP			///<í“¬‚‘¬‰»ÀŒ±ƒtƒ‰ƒO
...
/**
 *	–½—ßƒe[ƒuƒ‹
 *	waza_seq_def.h‚ÌƒCƒ“ƒfƒbƒNƒX‚Æˆê‘Î‚É‚È‚é‚æ‚¤‚É‚·‚é
 */
//============================================================================================
...
//============================================================================================
/**
 *	ƒGƒ“ƒJƒEƒ“ƒg—pƒ|ƒPƒ‚ƒ“‚ğƒZƒbƒg
 *
 * @param[in]	bw	í“¬—pƒ[ƒN\‘¢‘Ì
 * @param[in]	sp	ƒT[ƒo—pƒpƒ‰ƒ[ƒ^ƒ[ƒN\‘¢‘Ì
 *
 * ƒf[ƒ^ˆø”F
 * 0:ƒGƒ“ƒJƒEƒ“ƒgƒ|ƒPƒ‚ƒ“‚ğƒZƒbƒg‚·‚é‘¤‚Ìw’è(SIDE_ALL:‚·‚×‚Ä SIDE_MINE:©•ª‘¤ SIDE_ENEMYF‘Šè‘¤)
 */
//============================================================================================
static	BOOL	WS_POKEMON_ENCOUNT(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
...
static	void	DamageCalcAct(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	sp->damage*=sp->critical;		//ƒNƒŠƒeƒBƒJƒ‹ƒqƒbƒg‚É‚æ‚éƒ_ƒ[ƒWŒvZ

	//‘•”õŒø‰ÊˆĞ—ÍƒAƒbƒv‚ÍAˆĞ—Í“ƒ_ƒ[ƒW—ÊƒAƒbƒv
	...
	//‘•”õŒø‰Ê‚Å“¯‚¶‹Z‚ğ‘±‚¯‚Ä‚¢‚é‚ÉˆĞ—Í‚ğã‚°‚éê‡‚ÍAƒ_ƒ[ƒWƒAƒbƒv
...
static	BOOL	WS_ATTACK_MESSAGE(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	//˜A‘±UŒ‚‚Ì‚ÍAÅ‰‚Ì1‰ñ–Ú‚¾‚¯•\¦
	if((sp->loop_flag==0)&&
	//ƒAƒ^ƒbƒNƒƒbƒZ[ƒW•\¦‚È‚µƒtƒ‰ƒO‚ª‚½‚Á‚Ä‚¢‚é‚ÍA•\¦‚È‚µ
...
static	BOOL	WS_HP_CALC(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	//”½“®ƒ_ƒ[ƒW—p‚É‚g‚o‚ğŒ¸‚ç‚µ‚½ÀÛ—Ê‚ğŠi”[
...
static	BOOL	WS_WAIT(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	//’ÊM‘Îí‚Å‚È‚¢‚ÍAƒ{ƒ^ƒ“‚©ƒ^ƒbƒ`‚³‚ê‚½‚çAƒEƒGƒCƒg‚ğ‚È‚­‚·
	...
	if(wait>sp->wait_cnt){
		//ƒV[ƒPƒ“ƒXƒJƒEƒ“ƒ^‚ğ–½—ßƒR[ƒh‚Ü‚Å‚Ü‚«–ß‚µ
		SkillSeqInc(sp,-2);
...
static	BOOL	WS_IF(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	switch(act){
	case IF_FLAG_EQ:		//ˆê’v
		if(data[0]!=dest){
			adrs=0;
		}
		break;
	case IF_FLAG_NE:		//•sˆê’v
		if(data[0]==dest){
			adrs=0;
		}
		break;
	case IF_FLAG_C:			//’´‚¦‚é
		...
		break;
	case IF_FLAG_NC:		//ˆÈ‰º
		if(data[0]>dest){
			adrs=0;
		}
		break;
	case IF_FLAG_BIT:		//ƒrƒbƒgON
		if((data[0]&dest)==0){
			adrs=0;
		}
		break;
	case IF_FLAG_NBIT:		//ƒrƒbƒgOFF
		if(data[0]&dest){
			adrs=0;
		}
		break;
	case IF_FLAG_AND:		//ƒrƒbƒgMASK‚µ‚ÄŠ®‘Sˆê’v‚·‚é‚©H
		if((data[0]&dest)!=dest){
			adrs=0;
		}
		break;
	default:
		GF_ASSERT_MSG(TRUE,"•s³‚È’è‹`‚Å‚·\n");
		break;
	}
	if(adrs){
		SkillSeqInc(sp,adrs);
...
static	BOOL	WS_CRITICAL_CHECK(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	//Å‰‚Ìí“¬‚ÆA•ßŠlƒfƒ‚‚Å‚ÍA‹}Š‚É‚ ‚½‚ç‚È‚¢
...
static	BOOL	WS_GET_EXP_CHECK(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	//ŒoŒ±’lƒQƒbƒgˆ—‚ğ‚µ‚È‚¢‚Æ‚«‚Ì‚Æ‚Ñæ‚ğƒ[ƒh
	adrs=SkillSeqDataRead(sp);
	...
	if((cp->client_type&CLIENT_ENEMY_FLAG)&&((fight_type&FIGHT_TYPE_NO_EXP)==0)){
		{
			...
			//è‚¿ƒ|ƒPƒ‚ƒ“‚ªŠwK‘•’u‚ğ‚à‚Á‚Ä‚¢‚é‚©ƒ`ƒFƒbƒN
...
static	BOOL	WS_GET_EXP_LOOP(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	//ŒoŒ±’lƒQƒbƒgˆ—‚ğƒ‹[ƒv‚µ‚È‚¢‚Æ‚«‚Ì‚Æ‚Ñæ‚ğƒ[ƒh
	adrs=SkillSeqDataRead(sp);
...
static	BOOL	WS_POKEMON_RESHUFFLE(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	switch(side){
	default:
		OS_Printf("WS_POKEMON_RESHUFFLE:‚±‚±‚É‚­‚é‚Ì‚Í‚¨‚©‚µ‚¢\n");
		break;
	case SIDE_ATTACK:
...
static	BOOL	WS_POKEMON_RESHUFFLE_LOOP(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	//ƒ|ƒPƒ‚ƒ““ü‚ê‘Ö‚¦ˆ—‚ğƒ‹[ƒv‚µ‚È‚¢‚Æ‚«‚Ì”ò‚Ñæ‚ğƒ[ƒh
	adrs=SkillSeqDataRead(sp);
...
static	BOOL	WS_RENZOKU(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	if(sp->renzoku_count_temp==0){
		if(para==0){
			//“Á«ƒXƒLƒ‹ƒŠƒ“ƒN‚ÍA˜A‘±UŒ‚ƒJƒEƒ“ƒ^‚ğÅ‘å‚É‚·‚é
...
static	BOOL	WS_VALUE(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	case VAL_BIT:		//OR
		data[0]|=value;
		break;
	case VAL_NBIT:		//’l‚ğƒrƒbƒg”½“]‚µ‚ÄAND
...
static	BOOL	WS_ABICNT_CALC(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	//abiritycnt‚ğã‚°‰º‚°o—ˆ‚È‚©‚Á‚½ƒtƒ‰ƒO‚ğ‰Šú‰»
	...
	if(value>0){
		if(psp->abiritycnt[COND_POW+para]==12){
			//abiritycnt‚ğã‚°‰º‚°o—ˆ‚È‚©‚Á‚½ƒtƒ‰ƒO‚ğƒZƒbƒg
		...
		if((sp->tsuika_flag&ADD_STATUS_NO_TOKUSEI)==0){
			//©•ª‚ğ”\—Í’á‰º‚³‚¹‚éê‡‚ÍA‹ZŒø‰Ê‚â“Á«‚Å‚Í–h‚ª‚È‚¢
			if(sp->attack_client!=sp->tsuika_client){
				//‚µ‚ë‚¢‚«‚è‚ÍA”\—Í’á‰º‚ğó‚¯‚È‚¢
				...
				//“Á«ƒNƒŠƒAƒ{ƒfƒBA‚µ‚ë‚¢‚¯‚Ş‚è‚ÍA”\—Í’á‰º‚ğó‚¯‚È‚¢
				...
				//“Á«‚·‚é‚Ç‚¢‚ß‚ÍA–½’†—¦‚ª‰º‚ª‚ç‚È‚¢
				//“Á«‚©‚¢‚è‚«ƒoƒTƒ~‚ÍAUŒ‚—Í‚ª‰º‚ª‚ç‚È‚¢
...
static	BOOL	WS_TOKUSEI_CHECK(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	//SIDE_ALL‚Ì‚ÍAê‚É‚Å‚Ä‚¢‚éƒ|ƒPƒ‚ƒ“‚·‚×‚Ä‚ğƒ`ƒFƒbƒN
	if(side==SIDE_ALL){
		{
			int	client_set_max;
...
static	BOOL	WS_TEXTURE(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	//“Á«ƒ}ƒ‹ƒ`ƒ^ƒCƒv‚Í©g‚Ìƒ^ƒCƒv‚ğ•Ï‰»‚³‚¹‚é‹Z‚ÍA¸”s‚É‚·‚é
	...
	//‚Á‚Ä‚¢‚é‹Z‚Ìƒ^ƒCƒv‚Ì’†‚©‚çAŒ»İ‚Ì©•ª‚Ìƒ^ƒCƒv‚Å‚Í‚È‚¢‚à‚Ì‚ª‚ ‚é‚©ƒ`ƒFƒbƒN
	for(i=0;i<cnt;i++){
	...
	//‚È‚©‚Á‚½ê‡‚ÍA‚¤‚Ü‚­‚«‚Ü‚ç‚ñ‚É‚·‚é
	if(i==cnt){
		SkillSeqInc(sp,adrs);
	}
	else{
		//‚Á‚Ä‚¢‚é‹Z‚Ìƒ^ƒCƒv‚©‚çA©•ª‚Ìƒ^ƒCƒv‚Å‚Í‚È‚¢‚à‚Ì‚ğƒ‰ƒ“ƒ_ƒ€‚Å‘I‘ğ
		do{
			do{
...
static	BOOL	WS_MONOMANE(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	if((ST_MonomaneNGCheck(sp->waza_no_old[sp->defence_client])==FALSE)||
	   ...
	   //‘Šè‚ª‹Z‚ğo‚·‚Ì‚ğ¸”s‚µ‚Ä‚¢‚é‚ÍA‚à‚Ì‚Ü‚Ë¸”s
...
static	BOOL	WS_KANASIBARI(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	if((sp->psp[sp->defence_client].wkw.kanashibari_wazano==0)&&
	   ...
	   (sp->waza_no_old[sp->defence_client])){
		...
		sp->psp[sp->defence_client].wkw.kanashibari_count=BattleWorkRandGet(bw)%4+3;	//‚Q`‚TiÀÛ‚Í‚R`‚U‚É‚·‚éj
	}
	else{
		SkillSeqInc(sp,adrs);
	}
...
static	BOOL	WS_ENCORE(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	if((sp->psp[sp->defence_client].wkw.encore_wazano==0)&&
	   ...
	   (sp->waza_no_old[sp->defence_client])){
		...
		sp->psp[sp->defence_client].wkw.encore_count=BattleWorkRandGet(bw)%5+2;		//‚Q`‚U
#else AFTER_MASTER_070221_BT_FIX
		sp->psp[sp->defence_client].wkw.encore_count=BattleWorkRandGet(bw)%5+3;		//‚Q`‚UiÀÛ‚ÍA‚R`‚Vj
		...
		if((ST_TameWazaCheck(sp,sp->waza_no_texture2[sp->attack_client]))&&
		   (sp->psp[sp->waza_no_texture2_client[sp->attack_client]].condition2&CONDITION2_KEEP)){
		...
		else{
			{
				...
				//ˆê‰A1000‰ñ‰ñ‚µ‚Äƒ^ƒCƒv‚ğæ“¾‚µ‚Ä‚İ‚é
				for(i=0;i<1000;i++){
				...
				//1000‰ñ‰ñ‚µ‚Ä‚àƒ_ƒ‚È‚Íƒe[ƒuƒ‹‚ğã‚©‚ç‚È‚ß‚Äˆê’v‚·‚é‚à‚Ì‚ğ’T‚·
				i=0;
...
//‚½‚ßŒn‚ÍA‹Z‚ğo‚µ‚½uŠÔ‚ÉPP‚ğŒ¸‚ç‚µ‚Ä‚¢‚é‚Ì‚ÅA‹Z‚ğŒp‘±‚·‚é‚±‚Æ‚Éd—l•ÏXiƒAƒhƒoƒ“ƒX”Å‚Æ‚Íˆá‚¢‚Ü‚·jby soga 2006.04.29
//			//PP‚ª‚O‚É‚È‚Á‚½‚çA‚½‚ßŒnƒtƒ‰ƒO‚ğ—‚Æ‚·
...
static	BOOL	WS_DOROBOU(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	//“G‘¤‚©‚ç‚Ì‚Ç‚ë‚Ú‚¤‚ÅA¸”s‚·‚éFightType‚ğƒ`ƒFƒbƒN
...
//============================================================================================
/**
 * WS_GUARD_SUCCESS_CHECK—pŠm—¦ƒe[ƒuƒ‹
 */
//============================================================================================
...
// “ü‚ê‘Ö‚¦u‚¨‚¢‚¤‚¿v”­“®‚ÉAƒ^[ƒ“ÅŒã‚Ìu‚Ü‚à‚évu‚İ‚«‚èv‚ª
// ¬Œ÷‚µ‚Ä‚µ‚Ü‚¤•s‹ï‡‚ğC³
...
static	BOOL	WS_GUARD_SUCCESS_CHECK(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	//©•ª‚ªÅŒã‚È‚ç¸”s‚É‚·‚éiç‚ê‚Ä‚àˆÓ–¡‚ª‚È‚¢‚Ì‚Åj
...
//		sp->psp[sp->attack_client].condition2|=CONDITION2_MIGAWARI;
		...
		//‘Oì‚Æ‚Ì®‡«‚Ì‚½‚ßAg‘ã‚í‚è‚ğo‚µ‚½‚ç’÷‚ß‹ZŒn‚©‚çŠJ•ú
...
static	BOOL	WS_HENSIN(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	//‹Z‚ÌPP‚Í‚T‚É‚·‚éiƒfƒtƒHƒ‹ƒg‚ª‚T–¢–‚Ì‚à‚Ì‚ÍA‚»‚Ì‚Ü‚Ü‚Ì’l‚ğ“ü‚ê‚éj
	   ...
	   (ST_ServerTokuseiCheck(bw,sp,STC_HAVE_ALL_HP,0,TOKUSYU_EAROKKU)==0)){
		...
		if(sp->field_condition&FIELD_CONDITION_HARE_ALL){
			if((sp->psp[client_no].hp)&&
			  ((sp->psp[client_no].waza_kouka&(WAZAKOUKA_ANAWOHORU|WAZAKOUKA_DAIBINGU))==0)){
				//“Á«‚©‚ñ‚»‚¤‚Í‚¾AƒTƒ“ƒpƒ[‚ÍA‚Í‚ê‚ÌAHPMAX‚Ì1/8ƒ_ƒ[ƒW
		...
		if(sp->field_condition&FIELD_CONDITION_ARARE_ALL){
			if((sp->psp[client_no].hp)&&
			  ((sp->psp[client_no].waza_kouka&(WAZAKOUKA_ANAWOHORU|WAZAKOUKA_DAIBINGU))==0)){
				//“Á«ƒAƒCƒXƒ{ƒfƒB‚ÍA‚ ‚ç‚ê‚ÌAHPMAX‚Ì1/16‰ñ•œ
...
static	BOOL	WS_WEATHER_KAIFUKU(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	//“VŒó•Ï‰»‚ª‚È‚¢‚©A“VŒó–³Œø‚É‚·‚é“Á«‚Ìƒ|ƒPƒ‚ƒ“‚ª‚¢‚é‚ÍAMAX‚Ì”¼•ª
// ----------------------------------------------------------------------------
// localize_spec_mark(JP_VER10) imatake 2006/12/01
#if B1374_060817_FIX
...
static	BOOL	WS_TRICK(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	//“G‘¤‚©‚ç‚ÌƒgƒŠƒbƒN‚ÅA¸”s‚·‚éFightType‚ğƒ`ƒFƒbƒN
	if((BattleWorkMineEnemyCheck(bw,sp->attack_client))&&((fight_type&FIGHT_TYPE_DOROBOU)==0)){
		SkillSeqInc(sp,adrs1);
	}
	//’@‚«—‚Æ‚³‚ê‚Ä‚¢‚é‚ÍA¸”s‚·‚é
...
static	BOOL	WS_MAGIC_CORT_A_D(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	//ƒ}ƒWƒbƒNƒR[ƒg‚Å‚ÌAttackADefence“ü‚ê‘Ö‚¦ƒtƒ‰ƒO‚ğ—§‚Ä‚é
...
static	BOOL	WS_HUNKA(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	//2‘Ì‚É“–‚½‚é‹Z‚¾‚ÆAHP‚ªŒ¸‚Á‚½‚É2‘Ì–Ú‚ÅÄŒvZ‚³‚ê‚½‚ÉˆĞ—Í‚ª‚¸‚ê‚é‚Ì‚ÅA2‰ñ–Ú‚ÍƒpƒX‚·‚é‚æ‚¤‚É‚·‚é
...
static	BOOL	WS_HUUIN(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	//‚Ó‚¤‚¢‚ñ‚ğ‚½‚ß‚µ‚½ƒtƒ‰ƒO‚ğ—§‚Ä‚éi‹ZAI—pj
...
// Å‰‚Ìƒ^[ƒ“‚É“ü‚ê‘Ö‚¦u‚¨‚¢‚¤‚¿v‚·‚é‚Æ‚±‚¾‚í‚ç‚È‚¢•s‹ï‡‚ÌC³‚ğ”½‰f
...
static	BOOL	WS_OTF_CHECK(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	switch(flag){
	case OTF_waruagaki_flag:	//‚í‚é‚ ‚ª‚«ƒtƒ‰ƒO
		if(sp->otf[client_no].waruagaki_flag==value){
			ret=1;
		}
		break;
	case OTF_pp_dec_flag:		//PP‚ğŒ¸‚ç‚µ‚½ƒtƒ‰ƒO
		if(sp->otf[client_no].pp_dec_flag==value){
			ret=1;
		}
		break;
	case OTF_mamoru_flag:		//‚Ü‚à‚éƒtƒ‰ƒO
		if(sp->otf[client_no].mamoru_flag==value){
			ret=1;
		}
		break;
	case OTF_tedasuke_flag:		//‚Ä‚¾‚·‚¯ƒtƒ‰ƒO
...
static const u16 MonohiroiTable2[]={
	...
	ITEM_PIIPIIEIDO,
	ITEM_SIROIHAABU,
	ITEM_WAZAMASIN44,	//‚Ë‚Ş‚é
	ITEM_PIIPIIEIDAA,
	ITEM_WAZAMASIN01,	//‚«‚ ‚¢ƒpƒ“ƒ`
	ITEM_TABENOKOSI,
	ITEM_WAZAMASIN26,	//‚¶‚µ‚ñ
};

static	const u8 ItemProb[] = {		//E‚¤ƒAƒCƒeƒ€‚ÌŠm—¦
	30,		// 30%
	40,		// 10%
	50,		// 10%
	60,		// 10%
...
//============================================================================================
/**
 *	“Á«‚à‚Ì‚Ğ‚ë‚¢‚Ìƒ`ƒFƒbƒNi‚İ‚Â‚ ‚Â‚ßAƒ~ƒmƒ€ƒbƒ`‚ÌƒtƒHƒ‹ƒ€ƒ`ƒFƒ“ƒW‚àƒ^ƒCƒ~ƒ“ƒO‚Íˆê‚È‚Ì‚Å‡‚í‚¹‚Äƒ`ƒFƒbƒNj
 *
 * @param[in]	bw	í“¬—pƒ[ƒN\‘¢‘Ì
 * @param[in]	sp	ƒT[ƒo—pƒpƒ‰ƒ[ƒ^ƒ[ƒN\‘¢‘Ì
 */
...
static	BOOL	WS_MONOHIROI(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	for(i=0;i<BattleWorkPokeCountGet(bw,CLIENT_NO_MINE);i++){
		...
		if((speabi==TOKUSYU_MONOHIROI)&&(monsno!=0)&&(monsno!=MONSNO_TAMAGO)&&(itemno==0)&&((BattleWorkRandGet(bw)%10)==0)){
			...
			if(LvOffset>=10){
				LvOffset=9;//ƒŒƒxƒ‹‚ª100‚ğ’´‚¦‚È‚¯‚ê‚Î‚ ‚è‚¦‚È‚¢‚ªA”O‚Ì‚½‚ß
			}
			for(j=0;j<9;j++){
				if(ItemProb[j]>rnd){
		...
		if((speabi==TOKUSYU_MITUATUME)&&(monsno!=0)&&(monsno!=MONSNO_TAMAGO)&&(itemno==0)){
			...
			//ƒe[ƒuƒ‹ƒI[ƒo[‚ğŠÄ‹‚·‚é‚½‚ß‚ÉƒAƒT[ƒg‚ğ‚©‚Ü‚·
			GF_ASSERT(j<10);
...
static	const	u16	ShizennochikaraWazaTable[]={
	WAZANO_ZISIN,			//»—˜
	WAZANO_ZISIN,			//»•l
	WAZANO_TANEBAKUDAN,		//Å¶
	WAZANO_TANEBAKUDAN,		//…‚½‚Ü‚è
	WAZANO_IWANADARE,		//Šâ”§
	WAZANO_IWANADARE,		//“´ŒA
	WAZANO_HUBUKI,			//áŒ´
	WAZANO_HAIDOROPONPU,	//…ã
	WAZANO_REITOUBIIMU,		//•Xã
	WAZANO_TORAIATAKKU,		//°
	WAZANO_DOROBAKUDAN,		//À’n
	WAZANO_EASURASSYU,		//’İ‚è‹´
	WAZANO_TORAIATAKKU,		//‚»‚êˆÈŠOiˆê‰’è‹`j
};
...
static	const	u32	HimitsunochikaraAddTable[]={
	...
	ADD_COND_NEMURI|	ADD_STATUS_DEFENCE,		//Å¶
	ADD_COND_NEMURI|	ADD_STATUS_DEFENCE,		//…‚½‚Ü‚è
	ADD_COND2_HIRUMU|	ADD_STATUS_DEFENCE,		//Šâ”§
	ADD_COND2_HIRUMU|	ADD_STATUS_DEFENCE,		//“´ŒA
	ADD_COND_KOORI|		ADD_STATUS_DEFENCE,		//áŒ´
	ADD_COND2_POWDOWN|	ADD_STATUS_DEFENCE,		//…ã
	ADD_COND_KOORI|		ADD_STATUS_DEFENCE,		//•Xã
	ADD_COND_MAHI|		ADD_STATUS_DEFENCE,		//°
	ADD_COND2_AGIDOWN|	ADD_STATUS_DEFENCE,		//À’n
	ADD_COND2_AVOIDDOWN|ADD_STATUS_DEFENCE,		//’İ‚è‹´
	ADD_COND_MAHI|		ADD_STATUS_DEFENCE,		//‚»‚êˆÈŠOiˆê‰’è‹`j
};
...
static	BOOL	WS_STEALTHROCK_CHECK(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	if((sp->side_condition[dir]&SIDE_CONDITION_STEALTHROCK)&&(sp->psp[client_no].hp)){
		...
		case AISYOU_4BAI:		//4”{Œø‰Ê
			sp->hp_calc_work=2;
			break;
		case AISYOU_2BAI:		//2”{Œø‰Ê
			sp->hp_calc_work=4;
			break;
		case AISYOU_1BAI:		//1”{Œø‰Ê
			sp->hp_calc_work=8;
			break;
		case AISYOU_1_2BAI:		//1/2”{Œø‰Ê
			sp->hp_calc_work=16;
			break;
		case AISYOU_1_4BAI:		//1/4”{Œø‰Ê
			sp->hp_calc_work=32;
			break;
		case AISYOU_0BAI:		//Œø‰Ê‚È‚µ
			SkillSeqInc(sp,adrs);
			return FALSE;
		default:
			GF_ASSERT(0);
...
#ifdef PM_DEBUG
	//ƒfƒoƒbƒOƒ‚[ƒh‚Å‚¨‚µ‚á‚×‚èƒ‰ƒ“ƒNŒÅ’è‚Ìê‡‚ÍA‚»‚Ì’l‚Å’Ç‰ÁŒø‰ÊŠm—¦‚ğŒvZ
...
static	BOOL	WS_WAZA_HIT_TOKUSEI_CHECK(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	//‹Z‚ªƒqƒbƒg‚µ‚½‚Æ‚«‚Éƒ`ƒFƒbƒN‚·‚é“Á«‚ğƒ`ƒFƒbƒN
...
//============================================================================================
/**
 *	•Ê‚ÌƒV[ƒPƒ“ƒX‚ÉƒWƒƒƒ“ƒv
 *
 * @param[in]	sp			ƒT[ƒoƒ[ƒN\‘¢‘Ì
 * @param[in]	arc_kind	ƒV[ƒPƒ“ƒXƒf[ƒ^‚ÌƒA[ƒJƒCƒu‚Ìí—Ş
 * @param[in]	adrs		”ò‚ÑæƒV[ƒPƒ“ƒX‚ÌINDEX
 */
//============================================================================================
static	void	SkillSequenceJump(SERVER_PARAM *sp,int arc_kind,int adrs)
{
...
//============================================================================================
/**
 *	•Ê‚ÌƒV[ƒPƒ“ƒX‚ÉƒTƒuƒ‹[ƒ`ƒ“ƒWƒƒƒ“ƒv
 *
 * @param[in]	sp			ƒT[ƒoƒ[ƒN\‘¢‘Ì
 * @param[in]	arc_kind	ƒV[ƒPƒ“ƒXƒf[ƒ^‚ÌƒA[ƒJƒCƒu‚Ìí—Ş
 * @param[in]	adrs		”ò‚ÑæƒV[ƒPƒ“ƒX‚ÌINDEX
...
//============================================================================================
/**
 *	srcƒpƒ‰ƒ[ƒ^‚Éw’è‚³‚ê‚½ƒoƒbƒtƒ@‚Ìƒ|ƒCƒ“ƒ^‚ğæ“¾
 *
 * @param[in]	sp			ƒT[ƒoƒ[ƒN\‘¢‘Ì
 * @param[in]	arc_kind	ƒV[ƒPƒ“ƒXƒf[ƒ^‚ÌƒA[ƒJƒCƒu‚Ìí—Ş
 * @param[in]	adrs		”ò‚ÑæƒV[ƒPƒ“ƒX‚ÌINDEX
...
// ----------------------------------------------------------------------------
// localize_spec_mark(LANG_ALL) imatake 2007/02/10
// “ü‚ê‘Ö‚¦u‚¨‚¢‚¤‚¿v”­¶‚Éu‚©‚¢‚ª‚ç‚Ì‚·‚¸v‚ÌŒø‰Ê‚ªo‚È‚¢•s‹ï‡‚ÌC³
#if AFTER_MASTER_070202_BT2_FIX
...
// ----------------------------------------------------------------------------
	...
	OS_Printf("WS_SrcPointerGet:‚±‚±‚É‚­‚é‚Ì‚ÍŠÔˆá‚¢I\n");
	return NULL;
}
 ...
 *	ŒoŒ±’lŠl“¾—pTCB
 *
 * @param[in]	tcb		TCBƒVƒXƒeƒ€ƒ[ƒN
 * @param[in]	work	ƒ[ƒNƒGƒŠƒAƒ|ƒCƒ“ƒ^
 */
//============================================================================================
static	void	TCB_GetExp(TCB_PTR tcb,void *work)
{
	...
	case SEQ_GE_STATUS_PRINT1:
		{
			...
			for(i=0;i<6;i++){
				...
				// ƒpƒ‰ƒƒ^‘•ª‚Ì•\¦‚ğAVƒpƒ‰ƒƒ^‚Æ“¯‚¶ˆÊ’u‚É‰EŠñ‚¹
	...
	case SEQ_GE_WAZAWASURE_ACT_MSG4:
		...
		tsiw->work[GE_MSG_INDEX]=BattleMSG_Print(tsiw->bw,msg_m,&mp,BattleWorkConfigMsgSpeedGet(tsiw->bw));
		i=0;
		PokeParaPut(pp,ID_PARA_pp_count1+tsiw->work[GE_WAZAPOS],&i);		//pp_count‚ğƒNƒŠƒA
 ...
 *	“w—Í’lŒvZ
 *
 * @param[in]	pp		POKEMON_PARAM\‘¢‘Ì‚Ìƒ|ƒCƒ“ƒ^
 * @param[in]	mons_no	‚à‚ç‚¦‚é“w—Í’lŒvZ‚ÌŒ³‚Æ‚È‚éƒ|ƒPƒ‚ƒ“ƒiƒ“ƒo[
 * @param[in]	form_no	‚à‚ç‚¦‚é“w—Í’lŒvZ‚ÌŒ³‚Æ‚È‚éƒ|ƒPƒ‚ƒ“‚ÌƒtƒHƒ‹ƒ€ƒiƒ“ƒo[
 */
//============================================================================================
static	void	PokeExpCalc(POKEPARTY *ppt,int pos,int mons_no,int form_no)
{
...
enum{
	...
	POKEMON_INHALE_WAIT=23,	//ƒ|ƒPƒ‚ƒ“‹z‚¢‚Ş‚Ü‚Å‚ÌƒEƒGƒCƒg
	BALL_SHAKE_WAIT=12		//ƒ{[ƒ‹—h‚ç‚µ‚½Œã‚ÌƒEƒGƒCƒg
};

//============================================================================================
/**
 *	GET_POKEMON‚Ìtsiw->work_p‚Ì“Y‚¦š
 */
//============================================================================================
enum{
	GP_ZUKAN_P=0,
...
static	void	TCB_GetPokemon(TCB_PTR tcb,void *work)
{
		...
		if(tsiw->flag==GET_NORMAL){
			{
				...
				bmd.mode=EBMM_CAPTURE;		///< “G‚É“Š‚°‚é
				bmd.heap_id=HEAPID_BATTLE;
				bmd.id=get_client_no+20000;	///<¸”s‚Ì‚ÉPokemonAppear‚Ì•û‚Ìid‚Æ”í‚ç‚È‚¢‚æ‚¤‚É‚ ‚è‚¦‚È‚¢’l‚É‚µ‚Ä‚¨‚­
				...
				//ƒ{[ƒ‹“Š‚°‚½ƒJƒEƒ“ƒg‚ğƒCƒ“ƒNƒŠƒƒ“ƒg
				tsiw->bw->ball_count++;
		...
		if(PaletteFadeCheck(pfd)==0){
			{
				...
				//ƒƒbƒZ[ƒWƒEƒCƒ“ƒhƒEDSƒAƒCƒRƒ“ƒpƒŒƒbƒgƒAƒjƒÄŠJ
		...
		if(PaletteFadeCheck(pfd)==0){
			{
				...
				//•ß‚Ü‚¦‚½ƒ|ƒPƒ‚ƒ“‚Ì•C”‚ğƒJƒEƒ“ƒgƒAƒbƒv
				...
				if(PokeParty_Add(poke_party,pp)==TRUE){
				...
				else{
					{
						...
						//ƒ{ƒbƒNƒX“]‘—‚·‚é‚ÍAPP‚ğ‰ñ•œ‚µ‚Ä‚¨‚­
						for(i=0;i<WAZA_TEMOTI_MAX;i++){
							ppmax=PokeParaGet(pp,ID_PARA_pp_max1+i,NULL);
							PokeParaPut(pp,ID_PARA_pp1+i,&ppmax);
						}
						//“]‘—‚·‚éƒ|ƒPƒ‚ƒ“‚ªƒMƒ‰ƒeƒBƒi‚ÅA‚Í‚Á‚«‚ñ‚¾‚Ü‚ğŠ‚µ‚Ä‚È‚­‚ÄA
						//ƒtƒHƒ‹ƒ€‚ªƒIƒŠƒWƒ“‚Ì‚Æ‚«‚ÍAƒAƒiƒU[‚É‚·‚é
	...
	case SEQ_GP_END:
		if(PaletteFadeCheck(pfd)==0){
			//ƒ|ƒPƒp[ƒNA•ßŠlƒfƒ‚‚Ì‚ÍA‚±‚Ì‚Q‚Â‚ªíœ‚³‚ê‚Ä‚¢‚È‚¢‚Ì‚Åíœ
			if(BattleWorkFightTypeGet(tsiw->bw)&(FIGHT_TYPE_POKE_PARK|FIGHT_TYPE_GET_DEMO)){
				BM_Delete(tsiw->bms);
				SoftSpriteClear(ssm_p);
			}
...
static	int	PokemonGetCalc(BATTLE_WORK *bw,SERVER_PARAM *sp)
{
	...
	//ƒ|ƒPƒp[ƒNA•ßŠlƒfƒ‚‚È‚çA100%•ßŠl¬Œ÷
	if(BattleWorkFightTypeGet(bw)&(FIGHT_TYPE_POKE_PARK|FIGHT_TYPE_GET_DEMO)){
		return 4;
	}

//ƒTƒtƒ@ƒŠƒ{[ƒ‹‚È‚çƒGƒT‚Æƒhƒ‚ğ“Š‚°‚½‰ñ”‚Å•ßŠl—¦‚ğ•Ï“®
...
static	int	SideClientNoGet(BATTLE_WORK *bw,SERVER_PARAM *sp,int side)
{
	...
	switch(side){
	default:
		OS_Printf("SideClientNoGet:‚±‚±‚É‚­‚é‚Ì‚Í‚¨‚©‚µ‚¢\n");
	case SIDE_ATTACK:
		client_no=sp->attack_client;
		break;
	case SIDE_DEFENCE:
	...
	case SIDE_ATTACK_PAIR:
		{
			...
			if(client_no==client_set_max){
				GF_ASSERT("ƒp[ƒgƒi[‚ªŒ©‚Â‚©‚è‚Ü‚¹‚ñ‚Å‚µ‚½@SIDE_ATTACK_PAIR‚ğw’è‚Å‚«‚Ü‚¹‚ñI\n");
				client_no=0;
			}
		}
		break;
	case SIDE_DEFENCE_PAIR:
		{
			...
			if(client_no==client_set_max){
				GF_ASSERT("ƒp[ƒgƒi[‚ªŒ©‚Â‚©‚è‚Ü‚¹‚ñ‚Å‚µ‚½@SIDE_DEFENCE_PAIR‚ğw’è‚Å‚«‚Ü‚¹‚ñI\n");
				client_no=0;
			}
		}
		break;
...
//============================================================================================
/**
 *	MessageParamMake—p‚Ìƒpƒ‰ƒ[ƒ^\‘¢‘Ìƒf[ƒ^¶¬ŠÖ”
 *
 * @param[in]	sp		ƒT[ƒoƒpƒ‰ƒ[ƒ^\‘¢‘Ì
 */
//============================================================================================
static	void	MessageParamDataMake(SERVER_PARAM *sp,MESSAGE_PARAM_DATA *mpd)
{
	...
	case TAG_NICK_ITEM_NICK:				//nickname		“¹‹ï–¼			nickname
...
//============================================================================================
/**
 *	MESSAGE_PARAM\‘¢‘Ìƒf[ƒ^¶¬ŠÖ”
 *
 * @param[in]	bw		BATTLE_WORK\‘¢‘Ì‚Ö‚Ìƒ|ƒCƒ“ƒ^
 * @param[in]	sp		ƒT[ƒoƒpƒ‰ƒ[ƒ^\‘¢‘Ì
 * @param[in]	mpd		MESSAGE_PARAM_DATA\‘¢‘Ìƒf[ƒ^‚Ö‚Ìƒ|ƒCƒ“ƒ^
 * @param[in]	mp		MESSAGE_PARAM\‘¢‘Ìƒf[ƒ^‚Ö‚Ìƒ|ƒCƒ“ƒ^
 */
//============================================================================================
static	void	MessageParamMake(BATTLE_WORK *bw,SERVER_PARAM *sp,MESSAGE_PARAM_DATA *mpd,MESSAGE_PARAM *mp)
{
...
static	int	MessageParamWazaGet(SERVER_PARAM *sp,int para)
{
	...
	case SIDE_WORK:
		...
		break;
	default:
		OS_Printf("MessageParamWazaGet:‚±‚±‚É‚­‚é‚Ì‚Í‚¨‚©‚µ‚¢\n");
		break;
	}
	return ret;
}
...
static	int	MessageParamItemGet(SERVER_PARAM *sp,int para)
{
	...
	case SIDE_WORK:
		...
		break;
	default:
		OS_Printf("MessageParamItemGet:‚±‚±‚É‚­‚é‚Ì‚Í‚¨‚©‚µ‚¢\n");
		break;
	}
	return ret;
}
...
static	int	MessageParamNumGet(SERVER_PARAM *sp,int para)
{
	...
	case SIDE_WORK:
		...
		break;
	default:
		OS_Printf("MessageParamNumGet:‚±‚±‚É‚­‚é‚Ì‚Í‚¨‚©‚µ‚¢\n");
		break;
	}
	return ret;
}
...
static	int	MessageParamTypeGet(SERVER_PARAM *sp,int para)
{
	...
	case SIDE_WORK:
		...
		break;
	default:
		OS_Printf("MessageParamTypeGet:‚±‚±‚É‚­‚é‚Ì‚Í‚¨‚©‚µ‚¢\n");
		break;
	}
	return ret;
}
...
static	int	MessageParamStatusGet(SERVER_PARAM *sp,int para)
{
	...
	case SIDE_WORK:
		...
		break;
	default:
		OS_Printf("MessageParamStatusGet:‚±‚±‚É‚­‚é‚Ì‚Í‚¨‚©‚µ‚¢\n");
		break;
	}
	return ret;
}
...
static	int	MessageParamConditionGet(SERVER_PARAM *sp,int para)
{
	...
	case SIDE_WORK:
		...
		break;
	default:
		OS_Printf("MessageParamConditionGet:‚±‚±‚É‚­‚é‚Ì‚Í‚¨‚©‚µ‚¢\n");
		break;
	}
	return ret;
}
...
static	int	MessageParamPolockGet(SERVER_PARAM *sp,int para)
{
	...
	case SIDE_WORK:
		...
		break;
	default:
		OS_Printf("MessageParamPolockGet:‚±‚±‚É‚­‚é‚Ì‚Í‚¨‚©‚µ‚¢\n");
		break;
	}
	return ret;
}
...
static	int	MessageParamTasteGet(SERVER_PARAM *sp,int para)
{
	...
	case SIDE_WORK:
		...
		break;
	default:
		OS_Printf("MessageParamTasteGet:‚±‚±‚É‚­‚é‚Ì‚Í‚¨‚©‚µ‚¢\n");
		break;
	}
	return ret;
}
...
//============================================================================================
/**
 *	T‚¦ƒ|ƒPƒ‚ƒ“‚ÌƒŒƒxƒ‹ƒAƒbƒv‚É•\¦‚·‚éƒAƒCƒRƒ“‚Ì¶¬
 *
 * @param[in]	bw		í“¬ƒVƒXƒeƒ€\‘¢‘Ì
 * @param[in]	tsiw	TCB_SKILL_INTP_WORK\‘¢‘Ì
 * @param[in]	mons_no	¶¬‘ÎÛƒ‚ƒ“ƒXƒ^[ƒiƒ“ƒo[
 */
//============================================================================================
...
///ƒvƒŒ[ƒgƒAƒCƒRƒ“FƒAƒNƒ^[ƒwƒbƒ_
static const TCATS_OBJECT_ADD_PARAM_S PlateIconObjParam = {
	BENCH_LEVELUP_ICON_X, BENCH_LEVELUP_ICON_Y, 0,		//x, y, z
	0, BENCH_LEVELUP_ICON_PRI, 0,		//ƒAƒjƒ”Ô†A—Dæ‡ˆÊAƒpƒŒƒbƒg”Ô†
	NNS_G2D_VRAM_TYPE_2DMAIN,		//•`‰æƒGƒŠƒA
	{	//g—pƒŠƒ\[ƒXIDƒe[ƒuƒ‹
		CHARID_BENCH_LEVELUP_ICON,		//ƒLƒƒƒ‰
		PLTTID_BENCH_LEVELUP_ICON,		//ƒpƒŒƒbƒg
		CELLID_BENCH_LEVELUP_ICON,		//ƒZƒ‹
		CELLANMID_BENCH_LEVELUP_ICON,	//ƒZƒ‹ƒAƒjƒ
...
static const TCATS_OBJECT_ADD_PARAM_S PokeIconObjParam = {
	...
	0, BENCH_LEVELUP_POKEICON_PRI, 0,		//ƒAƒjƒ”Ô†A—Dæ‡ˆÊAƒpƒŒƒbƒg”Ô†
	NNS_G2D_VRAM_TYPE_2DMAIN,				//•`‰æƒGƒŠƒA
	{	//g—pƒŠƒ\[ƒXIDƒe[ƒuƒ‹
		CHARID_BENCH_LEVELUP_POKEICON,		//ƒLƒƒƒ‰
		PLTTID_BENCH_LEVELUP_POKEICON,		//ƒpƒŒƒbƒg
		CELLID_BENCH_LEVELUP_POKEICON,		//ƒZƒ‹
		CELLANMID_BENCH_LEVELUP_POKEICON,	//ƒZƒ‹ƒAƒjƒ
		CLACT_U_HEADER_DATA_NONE,			//ƒ}ƒ‹ƒ`ƒZƒ‹
		CLACT_U_HEADER_DATA_NONE,			//ƒ}ƒ‹ƒ`ƒZƒ‹ƒAƒjƒ
	},
	1,			//BGƒvƒ‰ƒCƒIƒŠƒeƒB
...
//============================================================================================
/**
 *	T‚¦ƒ|ƒPƒ‚ƒ“‚ÌƒŒƒxƒ‹ƒAƒbƒv‚É•\¦‚·‚éƒAƒCƒRƒ“‚Ìíœ
 *
 * @param[in]	bw		í“¬ƒVƒXƒeƒ€\‘¢‘Ì
 * @param[in]	tsiw	TCB_SKILL_INTP_WORK\‘¢‘Ì
 */
...
//=========================================================================
/**
 *	•m€‚Ì‚Æ‚«‚Ì‚È‚Â‚«“xŒvZƒ‹[ƒ`ƒ“
 *
 * @param[in]	bw				í“¬ƒVƒXƒeƒ€\‘¢‘Ì
 * @param[in]	kizetsu_client	•m€‚É‚È‚Á‚½ClientNo
 */
//=========================================================================
...
//=========================================================================
/**
 *	AI—p“Á«ƒ[ƒN‚É“Á«ƒiƒ“ƒo[‚ğƒZƒbƒg
 *
 * @param[in]	sp			ƒT[ƒoƒVƒXƒeƒ€\‘¢‘Ì
 * @param[in]	client_no	ƒZƒbƒg‚·‚éClientNo
 * @param[in]	speabino	ƒZƒbƒg‚·‚é“Á«ƒiƒ“ƒo[
 */
//=========================================================================
static	void	TokusyuNoSet(SERVER_PARAM *sp,u8 client_no,u8 speabino)
{
...
//=========================================================================
/**
 *	AI—p‘•”õƒAƒCƒeƒ€ƒ[ƒN‚ÉƒAƒCƒeƒ€ƒiƒ“ƒo[‚ğƒZƒbƒg
 *
 * @param[in]	sp			ƒT[ƒoƒVƒXƒeƒ€\‘¢‘Ì
 * @param[in]	client_no	ƒZƒbƒg‚·‚éClientNo
 * @param[in]	item_no		ƒZƒbƒg‚·‚éƒAƒCƒeƒ€ƒiƒ“ƒo[
 */
//=========================================================================
static	void	SoubiItemSet(SERVER_PARAM *sp,u8 client_no,u16 item_no)
{
